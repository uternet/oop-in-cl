...
+ 定义类的组织。 您可以方便地定义类的组织。 类从其直接超类继承结构和行为，这些超类以 `defclass` 形式列出。 CLOS 根据 `defclass` 表单中本地指定的约束自动计算一个类优先级列表。

+ 创建和初始化实例。 您可以使用 `make-instance` 创建实例。 CLOS 为您提供了对实例初始化方式的良好控制。 您可以指定一个槽应由一个 `initarg` 填充，并为槽和 `initargs` 提供默认初始值。 您可以通过专门的初始化实例来执行进一步的初始化。
