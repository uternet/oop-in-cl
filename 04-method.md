...
`defclass` 表单定义槽并控制槽是本地的还是共享的。槽选项`:allocation` 指定一个槽是本地的还是共享的。默认是 `:allocation :instance`，这意味着槽是本地的。要指定应该共享一个槽，请提供 `:allocation :class` 槽选项。
...
为泛型函数 `number-of-sides` 定义一个共享槽和一个读取器方法。前面给出的三角形的两种 `defclass` 表单显示了如何定义共享槽（使用槽选项 `:allocation :class` ）和读取器方法（指定槽选项 `:reader number-of-sides`）。
...
您还可以通过使用带有变量的 `setq` 或 `setf` 来写入槽的值。下表显示了如何在所示方法中的 `with-accessors` 表单的主体中使用变量：
...
下表显示了如何在所示方法中在 `with-slots` 表单的主体中使用变量：
...
请注意，当您使用 `with-slots` 或 `with-accessors` 时，实例表单只会被求值一次。
...
我们可以通过实现一个用于执行远程求值的简单网络协议来说明常见Lisp类型的定义方法。此程序使用户能够通过网络将 Lisp 表单发送到另一台计算机，并接收对该表单进行求值的结果。发送表单的机器称为客户机，执行计算并将结果返回给客户机的机器称为服务器。
...
我们首先描述如何使用远程求值程序。我们创建一个流 `remote-eval-stream`，并调用函数 `remote-eval` 向该流发送表单并接收结果。最后，我们关闭流： 
...
`remote-eval`实现客户端；它向服务器发送表单，接收对该表单求值的结果，然后返回结果。表单在服务器环境中进行求值，任何副作用都会在该环境中发生。此示例不传输打印输出，不传输多个返回值，也不处理服务器上表单求值生成的错误；可以添加这些功能中的任何一个。
...
`eval-server`实现了服务器端；它不断地读取发送给它的表单，求值表单，并返回结果。当流关闭时，此进程被终止。
...
专门化参数是`(eql object)`，其中 object 是对表单求值的结果。表单只在定义方法时求值一次。
...
在第一种情况下，对象是在定义方法时通过一次求值形式获得的。在最后一种情况下，所需的参数是非专门化的，这相当于专门化了类 `t`。任何参数都满足一个非专门化参数，因为每个 Lisp 对象都是 `t` 类型。
